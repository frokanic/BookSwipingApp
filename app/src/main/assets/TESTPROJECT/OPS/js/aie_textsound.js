...
AIE.TextSound = {
    ...
    launchTextandSound: function() {
        $(".aie-audio-text-rw").each(function(i) {
            ...
            $(this).attr('data-audio-seq', i);
            textsound.init();
            AIE.TextSound.textsoundObjects.push(textsound);
        });
    },

    stopAllTextSound: function(curobj) {
        for (var i = 0; i < AIE.TextSound.textsoundObjects.length; i++) {
            var obj = AIE.TextSound.textsoundObjects[i];
            // this update caused issue that you couldn't pause the audio
            // when tried to pause while it is playing, it didn't worked
            var cur_audio = $(curobj).attr('data-audio-seq');
            if (cur_audio != i) {
                obj.stopStoryLine();
                obj.handleStopEvent();
            }
        ...
        if (self.sequenceMode == true) {
            if (currentTime <= self.endTime) {
                ...
                if (self.counter >= self.textsoundInstructions.length) {
                    ...
                    self.stopStoryLine();
                    //remove continues highlight
                    self.removeContinuesHighlight();
                    return;
                }
    ...
    this.setupPlayEvent = function() {
        ...
        if (isiPad == false) {
            $(self.parent).find('.buttons-rw >.play').click(function() {
                //	AIE.TextSound.stopAllTextSound($(this).parents(".aie-audio-text-rw"));
    ...
    this.playStoryLine = function(start_time) {
        try {
            ...
            if (self.startTime > 0) {
                ...
                self.audioPlayer.play();
                //we need to do settimeout as audioPlayer.currentTime is not accessible
                //until audio has started playingSW
                setTimeout(function() {
                    try {
                        self.audioPlayer.pause();
                        self.audioPlayer.currentTime = self.startTime;
                        self.audioPlayer.play();
                        if (AIE.TextSound.audioRate) self.audioPlayer.playbackRate = AIE.TextSound.audioRate;
                    } catch (e) {
                        self.setToggle($(this), 'Play');
                    }
                }, 15);
                ...
                if (AIE.TextSound.audioRate) this.audioPlayer.playbackRate = AIE.TextSound.audioRate;
            }
        } catch (e) {
            console.log(e.message);
            this.audioPlayer.load();
    ...
    this.getAudioInstruction = function(counter, timeIndex) {
        ...
        this.setHighlightByID = function(spanid) {
            if (spanid) {
                ...
                $('#' + spanid).addClass('audio-played');
            }
        },
        this.removeContinuesHighlight = function() {
            $(this.parent).find('.audio-played').removeClass('audio-played');
            //$('#'+spanid).addClass('audio-played');
        },
        ...
        this.stopStoryLine = function() {
            ...
            try {
                ...
                setTimeout(function() {
                    //we need a timeout here as ipad does not set the currenttime to 0 in normal case
                    try {
